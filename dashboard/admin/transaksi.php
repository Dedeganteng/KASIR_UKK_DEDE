...
if (isset($_GET['meja'])) {
    $query_order = mysqli_query($kon, "SELECT * FROM tb_order WHERE nama_pelanggan = '$_GET[meja]' ORDER BY id_order DESC");
            ...
            <div class="row">
                        ...
                        <div class="det">
                            ...
                            <table>
                                ...
                                <tr style="margin-top: 20px;">
                                    ...
                                    <td>: <?= date('d-m-Y', $order['tanggal_order']) ?></td>
                                </tr>
                                <tr style="margin-top: 20px;">
                        ...
                        <?php if (isset($_GET['meja'])) : ?>
                            <tbody>
                                ...
                                foreach ($detail_order as $row) :
                                    $query_mas = mysqli_query($kon, "SELECT * FROM tb_produk WHERE id_produk = '$row[id_produk]'");
                                    $produk = mysqli_fetch_assoc($query_mas); ?>
                                    <tr class="text-small">
                                        <td><?= $i++; ?></td>
                                        <td><?= $produk['nama_produk'] ?></td>
                                        <td><?= $row['jumlah_dorder'] ?></td>
                                        <td>Rp. <?= rupiah($produk['harga_produk']) ?></td>
                                        <td>Rp. <?= rupiah($produk['harga_produk'] * $row['jumlah_dorder']) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        <?php else : ?>
                ...
                <div class="col-md-4">
                    <form action="fungsi/prosesTransaksi.php" method="POST">
                        <?php $kursi = mysqli_query($kon, "SELECT * FROM tb_pelanggan WHERE status != 0"); ?>
                            ...
                            <select class="form-control" onchange='location=this.value' required>
                                ...
                                <?php if (isset($_GET['meja'])) : ?>
                                    <?php foreach ($kursi as $kurs) : ?>
                                        <option value="index.php?meja=<?= $kurs['nama_pelanggan'] ?>" <?= $kurs['nama_pelanggan'] == $_GET['meja'] ? 'selected' : '' ?>><?= $kurs['nama_pelanggan'] ?></option>
                                    <?php endforeach; ?>

                                <?php else : ?>
                                    <?php foreach ($kursi as $kurs) : ?>
                                        <option value="index.php?meja=<?= $kurs['nama_pelanggan'] ?>"><?= $kurs['nama_pelanggan'] ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary btn-block">Bayar dan Cetak Struk</button>
                    </form>
                </div>
            </div>
        </div>
