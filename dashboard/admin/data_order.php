            ...
            ?>
                <tbody>
                    <tr>
                        ...
                        <td><?= $orders['nama_pelanggan'] ?></td>
                        <td><?= date('d-m-Y H:i', $orders['tanggal_order']) ?></td>
                        <td>Rp. <?= rupiah($hartot['hartot']) ?></td>
                        
                        <td><button type="button" class="btn-success text-danger"><?= $orders['keterangan_order'] ?></button></td>
                        <td>
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                foreach ($detail_order as $list_row) :
                    $produk = mysqli_query($kon, "SELECT * FROM tb_produk WHERE id_produk = '$list_row[id_produk]' ");
                    $query_produk = mysqli_fetch_assoc($produk);
                ?>
                <tr>
                    <td><?= $i; ?></td>
                    <td><?= $query_produk['nama_produk'] ?></td>
                    <td><span class="badge badge-success"><?= $list_row['keterangan_dorder'] ?></span></td>
                    <td>Rp. <?= rupiah($query_produk['harga_produk']) ?></td>
                    <td align="center"><?= $list_row['jumlah_dorder'] ?></td>
                    <td>Rp. <?= rupiah($query_produk['harga_produk'] * $list_row['jumlah_dorder']) ?></td>
                </tr>
              <?php $i++; endforeach; ?>
            </tbody>
            <tfoot>
